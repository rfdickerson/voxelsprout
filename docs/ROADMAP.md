...
| Land value / desirability overlay | ✅ | Toggleable data overlay, `m_dataOverlay == DataOverlay::LandValue` in `citybuilder_app.h` |
...
| Population/economy/pollution heatmaps | ✅ | The single `m_showLandValue` toggle became `DataOverlay` (`citybuilder_app.h`): four mutually-exclusive ground layers — Land Value, Pollution, Education, Health — selectable from `drawControls`'s button row or by cycling the `L` hotkey. `computeDesirability` (`citybuilder_app.cc`) already scattered amenity/nuisance influence per tile for land value; it now also tees industrial/traffic/power-plant nuisance into a `Tile::pollution` field and school+library / clinic reach into `Tile::eduCoverage` / `Tile::healthCoverage`, reusing the exact same influence radius per building from `buildingInfluence` so the overlay covers exactly what the sim covers. Coverage splats at a peak of 1.0 rather than borrowing `buildingInfluence`'s peak — that peak is a land-value weight (0.30 for a school), so reusing it would pin the Education layer to the bottom third of the ramp however many schools you built; pollution keeps the land-value weights (so its relative sources stay in the sim's proportions) and normalizes by `kPollutionFullScale`, the power plant's own peak, as the reference for "as bad as it gets". `drawWorldOverlay`'s legend swaps title/end-labels per layer; the red→green ramp direction (green = good) stays fixed across all four so switching layers doesn't require relearning the scale. Population itself was already covered by the existing Reports charts, not this per-tile overlay. |
| Regional play (multiple connected cities) | ⬜ | Not implemented — single city/map only |
| Modular building growth stages (SimCity 2013's signature) | ✅ | Confirmed via a full read of `procgen/building_generator.cc` + the zone-drawing path in `citybuilder_app.cc`: `Tile::develop` (0..3) maps to `level` (1..3), which doubles as architectural era — 1890s brick -> 1930s art-deco setback towers -> 1960s curtain-wall modernism (`generateBuilding`'s era switch) — with height/mass scaling directly off `level` (e.g. the setback tower's step count is `1 + level`). Residential additionally gets a wealth `tier` (0..2, driven by land value) that adds bay windows/porches/picket-fences/flower-beds. Growth isn't a pop-in: a framed job site (rising corner posts, a tower crane on plots >= 4 tiles, rebar/barrel clutter gated by `dev` thresholds) animates continuously up to `kConstructionDev`, then the finished mesh rises out of the ground via the existing `RisingBuilding` actor-stream animation the instant `level` increases — including on era upgrades, not just first appearance. Commercial/industrial plots at level >= 2 get roadside billboards. |
| Day/night lighting cycle | ✅ | Not a wishlist line item, but a visual-polish gap found while confirming the row above: `citybuilder_app.cc` already ran a full civic clock (`dayHour()`) driving rush hour, the school bus loop, and trash day, but `onRender` pinned the renderer's sun at one fixed elevation all game — the render side's own sunrise/sunset sky auto-tuning (`evaluateSunriseSkyTuning`, `RendererBackend::SkyDebugSettings::autoSunriseTuning`, on by default) sat completely unused. `sunElevationForHour()` now drives `m_renderer.setSunAngles`'s pitch from `dayHour()` every frame (a single cosine over the 24h clock, tuned so hour 9 lands exactly on the original fixed 38 deg so daytime play still reads the same); storm-gloom depression layers on top exactly as before, just relative to the time-of-day elevation instead of a hardcoded baseline. **Deliberately not a true night:** the arc bottoms out at a low golden 6 deg rather than dipping below the horizon, because a day here is 60 real seconds and `frame_run.cc`'s `isNight` branch hard-cuts the sun and drops to a 0.05/0.08/0.16 blue ambient the moment elevation crosses zero — a real night would black the city out for ~40% of every minute of play. Dawn/dusk still read warm because `computeSunColor` is already deep in its twilight tint at that elevation. |
...
| Skeletal animation / GPU skinning | 🟡 | CPU side done and tested: `src/anim/` (`Skeleton`/`AnimationClip`, `AnimationSampler` hierarchy + lerp/slerp keyframe evaluation, JSON loader), `odai_animation_tests` all passing. GPU side is wired into the actual frame: `recordSkinningPass` runs every frame before the shadow/prepass/main passes (`frame_run.cc`), each of those three passes has its own skinned-actor draw block reusing the existing `m_importedStaticPipeline` family (`frame_pass_shadow.cc`/`frame_pass_prepass.cc`/`frame_pass_main.cc`), `createSkinningComputeResources`/`destroySkinningComputeResources` are called from device init/shutdown (`init.cc`) alongside SSAO's, and it has real GPU timestamp query slots. It has since been extended from a single global "skinning slot" to `kMaxSkinnedInstances` (8) independent instance slots (`renderer_types.h`, `RendererBackend::SkinnedInstanceSlot` in `renderer_backend.h`) — each with its own descriptor-buffer-set, rest-pose/output buffers, and draws; `recordSkinningPass` dispatches once per active instance behind one shared pipeline, and the three consuming passes resolve each draw's vertex/index buffer per-draw (`m_bufferAllocator.getBuffer(draw.vertexBufferHandle/indexBufferHandle)`) rather than assuming one shared buffer, since different slots now legitimately have different buffers. `src/games/legion/` (below) is the first game to actually call this path end to end. Two real bugs were caught and fixed while first wiring the single-slot version in: the bone-matrix upload was missing the `transpose()` step the camera-MVP path always does before a `-matrix-layout-column-major` shader reads it, and `setSkinnedActorPose` was calling into the FrameArena before `beginFrame()` had run for that frame (fixed by splitting it into a cheap setter + a `uploadSkinnedActorPoseForFrame()` step called right after `beginFrame()`). A prior CI run also caught a genuine compile bug from a missing include order (`frame_pass_skinning.cc`/`skinning_resources.cc` needing `<GLFW/glfw3.h>` + `sim/network_procedural.h` before the shared-header include, matching every other `frame_pass_*.cc`). **Still not verified against a real Vulkan build** — the multi-instance extension above was written and reviewed in a sandbox with neither vcpkg nor a GPU (same constraint as the original single-slot work); the next real compile signal is CI or a real Windows/Linux+vcpkg build. Correcting an earlier inaccuracy in this doc: NIF skin data is not "parsed but discarded" — nothing skin/bone/keyframe-related is parsed anywhere in `import/fnv/nif_scene.cc` today; that import work is still fully ahead (see Explicitly Deferred below). |
...
| Real-time-with-pause tactical party combat | 🟡 | First slice: `src/games/legion/` — a Rome-themed 4-vs-4 skirmish (`LegionApp`, `engine::GameApp`). A procedural biped rig (`anim/biped_rig.h`, 21 bones) and idle/walk/attack clips drive a procedurally generated humanoid mesh (`procgen/humanoid_generator.h`, rigid single-bone-weight cylinder/box segments) through the multi-instance skinning path above — one instance per character, 8 total. Simple distance-based auto-battle AI (walk to engage range, attack on a fixed cycle, sink-and-freeze on death), space-bar pause, HP bars. No player unit control, no dialogue/approval hookup yet, and — like the skinning work it exercises — not verified against a real Vulkan build. Still much smaller in scope than the mass-battle rendering that's out of scope below (4-8 characters, not thousands) |
...
| Small-party skeletal animation + rendering (Dragon Age touchstone) | 🟡 | See Party RPG / Narrative below — CPU sampler done + tested, GPU skinning pipeline wired and now multi-instance (8 slots), first exercised end to end by `src/games/legion/`'s 4-vs-4 skirmish. `import/fnv/nif_scene.cc` still parses no skin/bone/keyframe data at all — real (non-procedural) character content is a separate, deferred import task. |
...
- Pollution/education/health-coverage heatmaps alongside the existing land-value overlay — **done.** `CityBuilderApp::DataOverlay` (`citybuilder_app.h`) replaces the old single `m_showLandValue` bool with four mutually-exclusive layers; `computeDesirability` (`citybuilder_app.cc`) now also fills `Tile::pollution`/`eduCoverage`/`healthCoverage` per tile from the same building-influence splats land value already used, so the layers are guaranteed to agree with the sim rather than being a separate approximation.
- Service coverage-radius modeling (fire/police/hospital) building on the existing siren-dispatch unit
- Confirm/finish modular building-growth visual tiers if not already complete — **done, was already built.** See the City Simulation table above for the full read-through of `building_generator.cc`'s era/level/tier system and the construction-site/rise-animation path; nothing needed to change.
...
**Tier 4 — Dragon Age foundation (biggest lift, infrastructure before features).** Sequenced bottom-up; three pieces are now in progress:
1. GPU skeletal animation (bone buffers + skinning shader) — **in progress, multi-instance.** `src/anim/` (skeleton/clip data model, hierarchy + keyframe sampler, JSON loader, plus a hand-authored procedural biped rig in `anim/biped_rig.h`) is done and tested. The GPU side (`ImportedSkinnedMeshVertex`, `skinning.comp.slang`, `skinning_resources.cc`, `frame_pass_skinning.cc`) is called from the frame every frame (`frame_run.cc`) and now supports `kMaxSkinnedInstances` (8) independent instance slots rather than one global slot — see the Party RPG / Narrative section above for the full extension notes. This was written without a Vulkan-capable build available (verify on a real build before relying on it); real NIF skin/keyframe import to drive it with actual Fallout NV character data is still a separate, deferred task (see below).
...
3. Companion approval/relationship state and small-party (4–8 character) real-time-with-pause combat loop — **persistence done, combat loop first slice done.** `dialogue_state_io.h/.cc` saves/loads `MapDialogueContext`'s flags and per-companion approval as JSON (file or in-memory string), so a game can carry approval across sessions once it owns a save slot to put it in. `src/games/legion/` is a first real-time-with-pause combat loop (4-vs-4, distance-based auto-battle AI, space to pause) built on the multi-instance skinning path above, but it predates any companion-roster/approval hookup — those two efforts haven't met yet. Still missing: a companion roster concept (names/portraits/who's actually in the party), any UI display of approval, and player unit control (Legion's combat is currently auto-battle only).
