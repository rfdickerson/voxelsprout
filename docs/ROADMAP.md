...
| Population/economy/pollution heatmaps | ✅ | The single `m_showLandValue` toggle became `DataOverlay` (`citybuilder_app.h`): four mutually-exclusive ground layers — Land Value, Pollution, Education, Health — selectable from `drawControls`'s button row or by cycling the `L` hotkey. `computeDesirability` (`citybuilder_app.cc`) already scattered amenity/nuisance influence per tile for land value; it now also tees industrial/traffic/power-plant nuisance into a `Tile::pollution` field and school+library / clinic reach into `Tile::eduCoverage` / `Tile::healthCoverage`, reusing the exact same radius/peak per building from `buildingInfluence` so the overlay matches what the sim actually uses. `drawWorldOverlay`'s legend swaps title/end-labels per layer; the red→green ramp direction (green = good) stays fixed across all four so switching layers doesn't require relearning the scale. Population itself was already covered by the existing Reports charts, not this per-tile overlay. |
