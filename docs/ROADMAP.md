...
| Skeletal animation / GPU skinning | 🟡 | CPU side is done and tested: `src/anim/` (`Skeleton`/`AnimationClip` data model, `AnimationSampler` hierarchy composition + lerp/slerp keyframe evaluation, JSON loader), `odai_animation_tests` all passing. GPU side is written but **not wired in**: `ImportedSkinnedMeshVertex`/`ImportedSkinnedMeshTemplate` (`renderer_types.h`), `skinning.comp.slang`, `skinning_resources.cc` (pipeline/descriptor-buffer setup, template upload, per-frame bone-matrix upload), `frame_pass_skinning.cc` (dispatch + explicit barrier) all exist but nothing calls them yet — see the integration checklist at the top of `skinning_resources.cc` for the exact remaining steps (call sites in `frame_run.cc`/`frame_pass_main.cc`, a matrix-convention check against the camera-MVP upload, real GPU timestamp query slots). Correcting an earlier inaccuracy in this doc: NIF skin data is not "parsed but discarded" — nothing skin/bone/keyframe-related is parsed anywhere in `import/fnv/nif_scene.cc` today; that import work is still fully ahead (see Explicitly Deferred below). |
...
3. Companion approval/relationship state and small-party (4–8 character) real-time-with-pause combat loop — approval read/write exists inside the dialogue runtime (`DialogueContext::approval`/`adjustApproval`); no persistence, UI, or combat loop yet.
