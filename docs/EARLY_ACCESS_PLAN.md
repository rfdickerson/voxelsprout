...
1. ⬜ Wire the already-linked Dear ImGui into `odai_ui_editor` for real inspector controls (color pickers, numeric drags, dropdowns) instead of the current hand-drawn step-buttons — probably the single biggest lever on "does this feel intuitive." The inspector is now generated from the `TypeDesc`/`PropDesc` schema table in `editor_document.h`, so this is a swap of the control-drawing layer, not a rewrite of what it edits.
