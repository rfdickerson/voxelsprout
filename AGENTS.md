...
documented in the `.Codex/skills/content-tools` skill — including `--sweep N`, the CPU
...
| `docs/SKYRIM_GRASS_AND_DISTANCE.md` | **Read before assuming a mod's records are inert** — load-order placement merging already works behind `--plugin-add`; what Skyrim grass needs instead (LTEX→GNAM→GRAS scatter, with the DATA layout); and why the horizon is empty |
...
The `.Codex/skills/new-game` skill scaffolds a new `src/games/<name>/` target; `.Codex/skills/vulkan-docs` checks current Vulkan practice before touching `render/backend/vulkan/`.
