...
VkFormat findSupportedNormalDepthFormat(VkPhysicalDevice physicalDevice) {
    constexpr std::array<VkFormat, 1> kCandidates = {
        VK_FORMAT_R16G16B16A16_SFLOAT
    };
...
bool Renderer::pickPhysicalDevice() {
    ...
    for (VkPhysicalDevice candidate : devices) {
        ...
        if (normalDepthFormat == VK_FORMAT_UNDEFINED) {
            std::cerr << "[render] skip GPU: no supported normal-depth format\n";
            continue;
        }
...
bool Renderer::createSwapchain() {
    ...
    if (!createNormalDepthTargets()) {
        std::cerr << "[render] normal-depth target creation failed\n";
        return false;
    }
    if (!createSsaoTargets()) {
        std::cerr << "[render] SSAO target creation failed\n";
        return false;
    }
...
bool Renderer::createNormalDepthTargets() {
    if (m_normalDepthFormat == VK_FORMAT_UNDEFINED) {
        std::cerr << "[render] normal-depth format is undefined\n";
        return false;
    }
    if (m_depthFormat == VK_FORMAT_UNDEFINED) {
        std::cerr << "[render] depth format is undefined for normal-depth prepass\n";
        return false;
    }

    const uint32_t imageCount = static_cast<uint32_t>(m_swapchainImages.size());
    m_normalDepthImages.assign(imageCount, VK_NULL_HANDLE);
    m_normalDepthImageMemories.assign(imageCount, VK_NULL_HANDLE);
    m_normalDepthImageViews.assign(imageCount, VK_NULL_HANDLE);
    m_normalDepthImageInitialized.assign(imageCount, false);
    m_normalDepthDepthImages.assign(imageCount, VK_NULL_HANDLE);
    m_normalDepthDepthImageMemories.assign(imageCount, VK_NULL_HANDLE);
    m_normalDepthDepthImageViews.assign(imageCount, VK_NULL_HANDLE);
    ...
    for (uint32_t i = 0; i < imageCount; ++i) {
        ...
        normalImageCreateInfo.format = m_normalDepthFormat;
        normalImageCreateInfo.extent.width = m_swapchainExtent.width;
        normalImageCreateInfo.extent.height = m_swapchainExtent.height;
        ...
        normalImageCreateInfo.usage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
        normalImageCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
        normalImageCreateInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;

        const VkResult normalImageResult =
            vkCreateImage(m_device, &normalImageCreateInfo, nullptr, &m_normalDepthImages[i]);
        if (normalImageResult != VK_SUCCESS) {
            logVkFailure("vkCreateImage(normalDepth)", normalImageResult);
            return false;
        }

        VkMemoryRequirements normalMemoryRequirements{};
        vkGetImageMemoryRequirements(m_device, m_normalDepthImages[i], &normalMemoryRequirements);
        const uint32_t normalMemoryTypeIndex = findMemoryTypeIndex(
            m_physicalDevice,
            normalMemoryRequirements.memoryTypeBits,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
        );
        if (normalMemoryTypeIndex == std::numeric_limits<uint32_t>::max()) {
            std::cerr << "[render] no memory type for normal-depth image\n";
            return false;
        }

        VkMemoryAllocateInfo normalAllocateInfo{};
        normalAllocateInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        normalAllocateInfo.allocationSize = normalMemoryRequirements.size;
        normalAllocateInfo.memoryTypeIndex = normalMemoryTypeIndex;

        const VkResult normalAllocResult =
            vkAllocateMemory(m_device, &normalAllocateInfo, nullptr, &m_normalDepthImageMemories[i]);
        if (normalAllocResult != VK_SUCCESS) {
            logVkFailure("vkAllocateMemory(normalDepth)", normalAllocResult);
            return false;
        }

        const VkResult normalBindResult =
            vkBindImageMemory(m_device, m_normalDepthImages[i], m_normalDepthImageMemories[i], 0);
        if (normalBindResult != VK_SUCCESS) {
            logVkFailure("vkBindImageMemory(normalDepth)", normalBindResult);
            return false;
        }

        VkImageViewCreateInfo normalViewCreateInfo{};
        normalViewCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        normalViewCreateInfo.image = m_normalDepthImages[i];
        normalViewCreateInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
        normalViewCreateInfo.format = m_normalDepthFormat;
        ...
        const VkResult normalViewResult =
            vkCreateImageView(m_device, &normalViewCreateInfo, nullptr, &m_normalDepthImageViews[i]);
        if (normalViewResult != VK_SUCCESS) {
            logVkFailure("vkCreateImageView(normalDepth)", normalViewResult);
            return false;
        }
        ...
        depthImageCreateInfo.extent.width = m_swapchainExtent.width;
        depthImageCreateInfo.extent.height = m_swapchainExtent.height;
        ...
        const VkResult depthImageResult =
            vkCreateImage(m_device, &depthImageCreateInfo, nullptr, &m_normalDepthDepthImages[i]);
        if (depthImageResult != VK_SUCCESS) {
            logVkFailure("vkCreateImage(normalDepthDepth)", depthImageResult);
            return false;
        }

        VkMemoryRequirements depthMemoryRequirements{};
        vkGetImageMemoryRequirements(m_device, m_normalDepthDepthImages[i], &depthMemoryRequirements);
        const uint32_t depthMemoryTypeIndex = findMemoryTypeIndex(
            m_physicalDevice,
            depthMemoryRequirements.memoryTypeBits,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
        );
        if (depthMemoryTypeIndex == std::numeric_limits<uint32_t>::max()) {
            std::cerr << "[render] no memory type for normal-depth prepass depth image\n";
            return false;
        }

        VkMemoryAllocateInfo depthAllocateInfo{};
        depthAllocateInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        depthAllocateInfo.allocationSize = depthMemoryRequirements.size;
        depthAllocateInfo.memoryTypeIndex = depthMemoryTypeIndex;

        const VkResult depthAllocResult =
            vkAllocateMemory(m_device, &depthAllocateInfo, nullptr, &m_normalDepthDepthImageMemories[i]);
        if (depthAllocResult != VK_SUCCESS) {
            logVkFailure("vkAllocateMemory(normalDepthDepth)", depthAllocResult);
            return false;
        }

        const VkResult depthBindResult =
            vkBindImageMemory(m_device, m_normalDepthDepthImages[i], m_normalDepthDepthImageMemories[i], 0);
        if (depthBindResult != VK_SUCCESS) {
            logVkFailure("vkBindImageMemory(normalDepthDepth)", depthBindResult);
            return false;
        }

        VkImageViewCreateInfo depthViewCreateInfo{};
        depthViewCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        depthViewCreateInfo.image = m_normalDepthDepthImages[i];
        ...
        const VkResult depthViewResult =
            vkCreateImageView(m_device, &depthViewCreateInfo, nullptr, &m_normalDepthDepthImageViews[i]);
        if (depthViewResult != VK_SUCCESS) {
            logVkFailure("vkCreateImageView(normalDepthDepth)", depthViewResult);
            return false;
        }
    }
...
bool Renderer::createSsaoTargets() {
    if (m_ssaoFormat == VK_FORMAT_UNDEFINED) {
        std::cerr << "[render] SSAO format is undefined\n";
        return false;
    }

    const uint32_t imageCount = static_cast<uint32_t>(m_swapchainImages.size());
    m_ssaoImages.assign(imageCount, VK_NULL_HANDLE);
    m_ssaoImageMemories.assign(imageCount, VK_NULL_HANDLE);
    m_ssaoImageViews.assign(imageCount, VK_NULL_HANDLE);
    m_ssaoImageInitialized.assign(imageCount, false);
    m_ssaoBlurImages.assign(imageCount, VK_NULL_HANDLE);
    m_ssaoBlurImageMemories.assign(imageCount, VK_NULL_HANDLE);
    m_ssaoBlurImageViews.assign(imageCount, VK_NULL_HANDLE);
    ...
    for (uint32_t i = 0; i < imageCount; ++i) {
        ...
        imageCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
        imageCreateInfo.imageType = VK_IMAGE_TYPE_2D;
        imageCreateInfo.format = m_ssaoFormat;
        imageCreateInfo.extent.width = m_swapchainExtent.width;
        imageCreateInfo.extent.height = m_swapchainExtent.height;
        imageCreateInfo.extent.depth = 1;
        imageCreateInfo.mipLevels = 1;
        ...
        const VkResult imageResult = vkCreateImage(m_device, &imageCreateInfo, nullptr, &m_ssaoImages[i]);
        if (imageResult != VK_SUCCESS) {
            logVkFailure("vkCreateImage(ssao)", imageResult);
            return false;
        }

        VkMemoryRequirements memoryRequirements{};
        vkGetImageMemoryRequirements(m_device, m_ssaoImages[i], &memoryRequirements);
        const uint32_t memoryTypeIndex = findMemoryTypeIndex(
            m_physicalDevice,
            memoryRequirements.memoryTypeBits,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
        ...
        if (memoryTypeIndex == std::numeric_limits<uint32_t>::max()) {
            std::cerr << "[render] no memory type for SSAO image\n";
            return false;
        }
        ...
        const VkResult allocResult = vkAllocateMemory(m_device, &allocateInfo, nullptr, &m_ssaoImageMemories[i]);
        if (allocResult != VK_SUCCESS) {
            logVkFailure("vkAllocateMemory(ssao)", allocResult);
            return false;
        }

        const VkResult bindResult = vkBindImageMemory(m_device, m_ssaoImages[i], m_ssaoImageMemories[i], 0);
        if (bindResult != VK_SUCCESS) {
            logVkFailure("vkBindImageMemory(ssao)", bindResult);
            return false;
        }

        VkImageViewCreateInfo viewCreateInfo{};
        viewCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        viewCreateInfo.image = m_ssaoImages[i];
        viewCreateInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
        viewCreateInfo.format = m_ssaoFormat;
        viewCreateInfo.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
        viewCreateInfo.subresourceRange.baseMipLevel = 0;
        viewCreateInfo.subresourceRange.levelCount = 1;
        viewCreateInfo.subresourceRange.baseArrayLayer = 0;
        ...
        const VkResult viewResult = vkCreateImageView(m_device, &viewCreateInfo, nullptr, &m_ssaoImageViews[i]);
        if (viewResult != VK_SUCCESS) {
            logVkFailure("vkCreateImageView(ssao)", viewResult);
            return false;
        }

        const VkResult blurImageResult = vkCreateImage(m_device, &imageCreateInfo, nullptr, &m_ssaoBlurImages[i]);
        if (blurImageResult != VK_SUCCESS) {
            logVkFailure("vkCreateImage(ssaoBlur)", blurImageResult);
            return false;
        }

        VkMemoryRequirements blurMemoryRequirements{};
        vkGetImageMemoryRequirements(m_device, m_ssaoBlurImages[i], &blurMemoryRequirements);
        const uint32_t blurMemoryTypeIndex = findMemoryTypeIndex(
            m_physicalDevice,
            blurMemoryRequirements.memoryTypeBits,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
        );
        if (blurMemoryTypeIndex == std::numeric_limits<uint32_t>::max()) {
            std::cerr << "[render] no memory type for SSAO blur image\n";
            return false;
        }

        VkMemoryAllocateInfo blurAllocateInfo{};
        blurAllocateInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        blurAllocateInfo.allocationSize = blurMemoryRequirements.size;
        blurAllocateInfo.memoryTypeIndex = blurMemoryTypeIndex;

        const VkResult blurAllocResult =
            vkAllocateMemory(m_device, &blurAllocateInfo, nullptr, &m_ssaoBlurImageMemories[i]);
        if (blurAllocResult != VK_SUCCESS) {
            logVkFailure("vkAllocateMemory(ssaoBlur)", blurAllocResult);
            return false;
        }

        const VkResult blurBindResult =
            vkBindImageMemory(m_device, m_ssaoBlurImages[i], m_ssaoBlurImageMemories[i], 0);
        if (blurBindResult != VK_SUCCESS) {
            logVkFailure("vkBindImageMemory(ssaoBlur)", blurBindResult);
            return false;
        }

        viewCreateInfo.image = m_ssaoBlurImages[i];
        const VkResult blurViewResult =
            vkCreateImageView(m_device, &viewCreateInfo, nullptr, &m_ssaoBlurImageViews[i]);
        if (blurViewResult != VK_SUCCESS) {
            logVkFailure("vkCreateImageView(ssaoBlur)", blurViewResult);
            return false;
        }
    }
...
bool Renderer::createAoPipelines() {
    ...
    if (m_normalDepthFormat == VK_FORMAT_UNDEFINED || m_ssaoFormat == VK_FORMAT_UNDEFINED || m_depthFormat == VK_FORMAT_UNDEFINED) {
        return false;
    }

    constexpr std::array<const char*, 4> kVoxelVertexShaderPathCandidates = {
        "src/render/shaders/voxel_packed.vert.spv",
        "../src/render/shaders/voxel_packed.vert.spv",
        "../../src/render/shaders/voxel_packed.vert.spv",
        "../../../src/render/shaders/voxel_packed.vert.spv",
    };
    constexpr std::array<const char*, 4> kVoxelNormalDepthFragmentShaderPathCandidates = {
    ...
    constexpr std::array<const char*, 4> kPipeNormalDepthFragmentShaderPathCandidates = {
        "src/render/shaders/pipe_normaldepth.frag.spv",
        "../src/render/shaders/pipe_normaldepth.frag.spv",
        "../../src/render/shaders/pipe_normaldepth.frag.spv",
        "../../../src/render/shaders/pipe_normaldepth.frag.spv",
    };
    constexpr std::array<const char*, 4> kFullscreenVertexShaderPathCandidates = {
        "src/render/shaders/tone_map.vert.spv",
        "../src/render/shaders/tone_map.vert.spv",
        "../../src/render/shaders/tone_map.vert.spv",
        "../../../src/render/shaders/tone_map.vert.spv",
    };
    constexpr std::array<const char*, 4> kSsaoFragmentShaderPathCandidates = {
        "src/render/shaders/ssao.frag.spv",
        "../src/render/shaders/ssao.frag.spv",
        "../../src/render/shaders/ssao.frag.spv",
        "../../../src/render/shaders/ssao.frag.spv",
    };
    constexpr std::array<const char*, 4> kSsaoBlurFragmentShaderPathCandidates = {
    ...
    if (!createShaderModuleFromFileOrFallback(
            m_device,
            kVoxelVertexShaderPathCandidates,
            nullptr,
            0,
            "voxel_packed.vert",
    ...
    if (!createShaderModuleFromFileOrFallback(
            m_device,
            kVoxelNormalDepthFragmentShaderPathCandidates,
            nullptr,
            0,
    ...
    if (!createShaderModuleFromFileOrFallback(
            m_device,
            kPipeNormalDepthFragmentShaderPathCandidates,
            nullptr,
            0,
    ...
    if (!createShaderModuleFromFileOrFallback(
            m_device,
            kFullscreenVertexShaderPathCandidates,
            nullptr,
            0,
            "tone_map.vert",
    ...
    if (!createShaderModuleFromFileOrFallback(
            m_device,
            kSsaoFragmentShaderPathCandidates,
            nullptr,
            0,
            "ssao.frag",
    ...
    if (!createShaderModuleFromFileOrFallback(
            m_device,
            kSsaoBlurFragmentShaderPathCandidates,
            nullptr,
            0,
            "ssao_blur.frag",
    ...
    VkPipelineShaderStageCreateInfo voxelVertexShaderStage{};
    voxelVertexShaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    voxelVertexShaderStage.stage = VK_SHADER_STAGE_VERTEX_BIT;
    voxelVertexShaderStage.module = voxelVertShaderModule;
    voxelVertexShaderStage.pName = "main";

    VkPipelineShaderStageCreateInfo voxelFragmentShaderStage{};
    voxelFragmentShaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    voxelFragmentShaderStage.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
    voxelFragmentShaderStage.module = voxelNormalDepthFragShaderModule;
    voxelFragmentShaderStage.pName = "main";

    const std::array<VkPipelineShaderStageCreateInfo, 2> voxelShaderStages = {
        voxelVertexShaderStage,
        voxelFragmentShaderStage
    };

    VkVertexInputBindingDescription voxelBindingDescription{};
    voxelBindingDescription.binding = 0;
    voxelBindingDescription.stride = sizeof(world::PackedVoxelVertex);
    voxelBindingDescription.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;

    VkVertexInputAttributeDescription voxelAttributeDescription{};
    voxelAttributeDescription.location = 0;
    voxelAttributeDescription.binding = 0;
    voxelAttributeDescription.format = VK_FORMAT_R32_UINT;
    voxelAttributeDescription.offset = 0;
    ...
    voxelVertexInputInfo.pVertexBindingDescriptions = &voxelBindingDescription;
    voxelVertexInputInfo.vertexAttributeDescriptionCount = 1;
    voxelVertexInputInfo.pVertexAttributeDescriptions = &voxelAttributeDescription;
    ...
    VkGraphicsPipelineCreateInfo voxelPipelineCreateInfo{};
    voxelPipelineCreateInfo.sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;
    voxelPipelineCreateInfo.pNext = &normalDepthRenderingCreateInfo;
    voxelPipelineCreateInfo.stageCount = static_cast<uint32_t>(voxelShaderStages.size());
    voxelPipelineCreateInfo.pStages = voxelShaderStages.data();
    voxelPipelineCreateInfo.pVertexInputState = &voxelVertexInputInfo;
    voxelPipelineCreateInfo.pInputAssemblyState = &inputAssembly;
    voxelPipelineCreateInfo.pViewportState = &viewportState;
    voxelPipelineCreateInfo.pRasterizationState = &rasterizer;
    voxelPipelineCreateInfo.pMultisampleState = &multisampling;
    voxelPipelineCreateInfo.pDepthStencilState = &depthStencil;
    voxelPipelineCreateInfo.pColorBlendState = &colorBlending;
    voxelPipelineCreateInfo.pDynamicState = &dynamicState;
    voxelPipelineCreateInfo.layout = m_pipelineLayout;
    voxelPipelineCreateInfo.renderPass = VK_NULL_HANDLE;
    voxelPipelineCreateInfo.subpass = 0;
    ...
    pipeFragmentShaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    pipeFragmentShaderStage.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
    pipeFragmentShaderStage.module = pipeNormalDepthFragShaderModule;
    pipeFragmentShaderStage.pName = "main";
    ...
    VkVertexInputAttributeDescription pipeAttributes[5]{};
    ...
    pipeVertexInputInfo.vertexAttributeDescriptionCount = 5;
    pipeVertexInputInfo.pVertexAttributeDescriptions = pipeAttributes;

    VkGraphicsPipelineCreateInfo pipePipelineCreateInfo = voxelPipelineCreateInfo;
    pipePipelineCreateInfo.stageCount = static_cast<uint32_t>(pipeShaderStages.size());
    pipePipelineCreateInfo.pStages = pipeShaderStages.data();
    pipePipelineCreateInfo.pVertexInputState = &pipeVertexInputInfo;
    VkPipelineRasterizationStateCreateInfo pipeRasterizer = rasterizer;
    pipeRasterizer.cullMode = VK_CULL_MODE_NONE;
    pipePipelineCreateInfo.pRasterizationState = &pipeRasterizer;
    ...
    VkPipelineShaderStageCreateInfo ssaoVertexShaderStage{};
    ssaoVertexShaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    ssaoVertexShaderStage.stage = VK_SHADER_STAGE_VERTEX_BIT;
    ssaoVertexShaderStage.module = fullscreenVertShaderModule;
    ssaoVertexShaderStage.pName = "main";

    VkPipelineShaderStageCreateInfo ssaoFragmentShaderStage{};
    ssaoFragmentShaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    ssaoFragmentShaderStage.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
    ssaoFragmentShaderStage.module = ssaoFragShaderModule;
    ssaoFragmentShaderStage.pName = "main";

    const std::array<VkPipelineShaderStageCreateInfo, 2> ssaoShaderStages = {
        ssaoVertexShaderStage,
        ssaoFragmentShaderStage
    };

    VkPipelineVertexInputStateCreateInfo ssaoVertexInputInfo{};
    ssaoVertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;

    VkPipelineDepthStencilStateCreateInfo ssaoDepthStencil{};
    ssaoDepthStencil.sType = VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO;
    ssaoDepthStencil.depthTestEnable = VK_FALSE;
    ssaoDepthStencil.depthWriteEnable = VK_FALSE;
    ...
    ssaoPipelineCreateInfo.stageCount = static_cast<uint32_t>(ssaoShaderStages.size());
    ssaoPipelineCreateInfo.pStages = ssaoShaderStages.data();
    ssaoPipelineCreateInfo.pVertexInputState = &ssaoVertexInputInfo;
    ssaoPipelineCreateInfo.pInputAssemblyState = &inputAssembly;
    ssaoPipelineCreateInfo.pViewportState = &viewportState;
    VkPipelineRasterizationStateCreateInfo ssaoRasterizer = rasterizer;
    ssaoRasterizer.cullMode = VK_CULL_MODE_NONE;
    ssaoPipelineCreateInfo.pRasterizationState = &ssaoRasterizer;
    ssaoPipelineCreateInfo.pMultisampleState = &multisampling;
    ssaoPipelineCreateInfo.pDepthStencilState = &ssaoDepthStencil;
    ...
    VkPipelineShaderStageCreateInfo ssaoBlurFragmentShaderStage{};
    ssaoBlurFragmentShaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    ssaoBlurFragmentShaderStage.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
    ssaoBlurFragmentShaderStage.module = ssaoBlurFragShaderModule;
    ssaoBlurFragmentShaderStage.pName = "main";

    const std::array<VkPipelineShaderStageCreateInfo, 2> ssaoBlurShaderStages = {
        ssaoVertexShaderStage,
        ssaoBlurFragmentShaderStage
    };

    VkGraphicsPipelineCreateInfo ssaoBlurPipelineCreateInfo = ssaoPipelineCreateInfo;
    ssaoBlurPipelineCreateInfo.stageCount = static_cast<uint32_t>(ssaoBlurShaderStages.size());
    ssaoBlurPipelineCreateInfo.pStages = ssaoBlurShaderStages.data();
    ...
    if (m_normalDepthVoxelPipeline != VK_NULL_HANDLE) {
        vkDestroyPipeline(m_device, m_normalDepthVoxelPipeline, nullptr);
    }
    if (m_normalDepthPipePipeline != VK_NULL_HANDLE) {
        vkDestroyPipeline(m_device, m_normalDepthPipePipeline, nullptr);
    }
    ...
    m_normalDepthVoxelPipeline = voxelNormalDepthPipeline;
    m_normalDepthPipePipeline = pipeNormalDepthPipeline;
...
void Renderer::renderFrame(
    ...
    ssaoImageInfo.sampler = m_ssaoSampler;
    ssaoImageInfo.imageView = m_ssaoBlurImageViews[imageIndex];
    ...
    ssaoRawImageInfo.imageView = m_ssaoImageViews[imageIndex];
    ...
    writes[6].pImageInfo = &ssaoImageInfo;
    ...
    transitionImageLayout(
        ...
        m_normalDepthImageInitialized[imageIndex] ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        m_normalDepthImageInitialized[imageIndex] ? VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT : VK_PIPELINE_STAGE_2_NONE,
        m_normalDepthImageInitialized[imageIndex] ? VK_ACCESS_2_SHADER_SAMPLED_READ_BIT : VK_ACCESS_2_NONE,
    ...
    transitionImageLayout(
        commandBuffer,
        m_normalDepthDepthImages[imageIndex],
        VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL,
        VK_PIPELINE_STAGE_2_NONE,
        VK_ACCESS_2_NONE,
    ...
    normalDepthClearValue.color.float32[2] = 1.0f;
    ...
    VkClearValue normalDepthDepthClearValue{};
    normalDepthDepthClearValue.depthStencil.depth = 0.0f;
    normalDepthDepthClearValue.depthStencil.stencil = 0;

    VkRenderingAttachmentInfo normalDepthDepthAttachment{};
    normalDepthDepthAttachment.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO;
    normalDepthDepthAttachment.imageView = m_normalDepthDepthImageViews[imageIndex];
    normalDepthDepthAttachment.imageLayout = VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL;
    normalDepthDepthAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
    normalDepthDepthAttachment.storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
    normalDepthDepthAttachment.clearValue = normalDepthDepthClearValue;
    ...
    normalDepthRenderingInfo.renderArea.extent = m_swapchainExtent;
    ...
    normalDepthRenderingInfo.pDepthAttachment = &normalDepthDepthAttachment;
    ...
    if (m_normalDepthVoxelPipeline != VK_NULL_HANDLE) {
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_normalDepthVoxelPipeline);
        vkCmdBindDescriptorSets(
            commandBuffer,
            VK_PIPELINE_BIND_POINT_GRAPHICS,
            m_pipelineLayout,
    ...
    if (m_normalDepthPipePipeline != VK_NULL_HANDLE && pipeInstanceCount > 0 && pipeInstanceSliceOpt.has_value()) {
        const VkBuffer pipeVertexBuffer = m_bufferAllocator.getBuffer(m_pipeVertexBufferHandle);
        const VkBuffer pipeIndexBuffer = m_bufferAllocator.getBuffer(m_pipeIndexBufferHandle);
        const VkBuffer pipeInstanceBuffer = m_bufferAllocator.getBuffer(pipeInstanceSliceOpt->buffer);
        if (pipeVertexBuffer != VK_NULL_HANDLE &&
            ...
            vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_normalDepthPipePipeline);
            vkCmdBindDescriptorSets(
                commandBuffer,
                VK_PIPELINE_BIND_POINT_GRAPHICS,
                m_pipelineLayout,
    ...
    transitionImageLayout(
        commandBuffer,
        m_ssaoImages[imageIndex],
        m_ssaoImageInitialized[imageIndex] ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        m_ssaoImageInitialized[imageIndex] ? VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT : VK_PIPELINE_STAGE_2_NONE,
        m_ssaoImageInitialized[imageIndex] ? VK_ACCESS_2_SHADER_SAMPLED_READ_BIT : VK_ACCESS_2_NONE,
        VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT,
        VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT,
        VK_IMAGE_ASPECT_COLOR_BIT
    );
    ...
    ssaoColorAttachment.imageView = m_ssaoImageViews[imageIndex];
    ...
    ssaoColorAttachment.clearValue = ssaoClearValue;
    ...
    ssaoRenderingInfo.renderArea.extent = m_swapchainExtent;
    ...
    transitionImageLayout(
        ...
        m_ssaoBlurImageInitialized[imageIndex] ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        m_ssaoBlurImageInitialized[imageIndex] ? VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT : VK_PIPELINE_STAGE_2_NONE,
        m_ssaoBlurImageInitialized[imageIndex] ? VK_ACCESS_2_SHADER_SAMPLED_READ_BIT : VK_ACCESS_2_NONE,
        VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT,
        VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT,
        VK_IMAGE_ASPECT_COLOR_BIT
    );

    VkRenderingAttachmentInfo ssaoBlurColorAttachment{};
    ssaoBlurColorAttachment.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO;
    ssaoBlurColorAttachment.imageView = m_ssaoBlurImageViews[imageIndex];
    ssaoBlurColorAttachment.imageLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    ssaoBlurColorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
    ssaoBlurColorAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
    ssaoBlurColorAttachment.clearValue = ssaoClearValue;
    ...
    ssaoBlurRenderingInfo.renderArea.extent = m_swapchainExtent;
    ssaoBlurRenderingInfo.layerCount = 1;
    ssaoBlurRenderingInfo.colorAttachmentCount = 1;
    ssaoBlurRenderingInfo.pColorAttachments = &ssaoBlurColorAttachment;
    ...
    m_ssaoImageInitialized[imageIndex] = true;
...
void Renderer::destroyNormalDepthTargets() {
    if (m_normalDepthSampler != VK_NULL_HANDLE) {
    ...
    for (VkImageView imageView : m_normalDepthDepthImageViews) {
        if (imageView != VK_NULL_HANDLE) {
            vkDestroyImageView(m_device, imageView, nullptr);
        }
    }
    m_normalDepthDepthImageViews.clear();

    for (VkImage image : m_normalDepthDepthImages) {
        if (image != VK_NULL_HANDLE) {
            vkDestroyImage(m_device, image, nullptr);
        }
    }
    m_normalDepthDepthImages.clear();

    for (VkDeviceMemory memory : m_normalDepthDepthImageMemories) {
        if (memory != VK_NULL_HANDLE) {
            vkFreeMemory(m_device, memory, nullptr);
        }
    }
    m_normalDepthDepthImageMemories.clear();
}

void Renderer::destroySsaoTargets() {
    if (m_ssaoSampler != VK_NULL_HANDLE) {
        vkDestroySampler(m_device, m_ssaoSampler, nullptr);
        m_ssaoSampler = VK_NULL_HANDLE;
    }

    for (VkImageView imageView : m_ssaoImageViews) {
        if (imageView != VK_NULL_HANDLE) {
            vkDestroyImageView(m_device, imageView, nullptr);
        }
    }
    m_ssaoImageViews.clear();

    for (VkImage image : m_ssaoImages) {
        if (image != VK_NULL_HANDLE) {
            vkDestroyImage(m_device, image, nullptr);
        }
    }
    ...
    for (VkDeviceMemory memory : m_ssaoImageMemories) {
        if (memory != VK_NULL_HANDLE) {
            vkFreeMemory(m_device, memory, nullptr);
        }
    }
    m_ssaoImageMemories.clear();
    m_ssaoImageInitialized.clear();
...
void Renderer::destroyPipeline() {
    ...
    if (m_normalDepthPipePipeline != VK_NULL_HANDLE) {
        vkDestroyPipeline(m_device, m_normalDepthPipePipeline, nullptr);
        m_normalDepthPipePipeline = VK_NULL_HANDLE;
    }
    if (m_normalDepthVoxelPipeline != VK_NULL_HANDLE) {
        vkDestroyPipeline(m_device, m_normalDepthVoxelPipeline, nullptr);
        m_normalDepthVoxelPipeline = VK_NULL_HANDLE;
    }
    if (m_tonemapPipeline != VK_NULL_HANDLE) {
        vkDestroyPipeline(m_device, m_tonemapPipeline, nullptr);
