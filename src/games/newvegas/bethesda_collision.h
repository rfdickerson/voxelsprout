...
public:
    ...
    // Recover a capsule whose feet were restored below a floor while its head
    // was above it. This can happen when an old traversal save or an explicit
    // diagnostic spawn is placed before the streamed bridge/floor collision is
    // resident. Jolt cannot choose a useful slide direction when a two-sided
    // mesh already cuts through the whole body, so move the feet onto that
    // intervening walkable surface before registering the controller.
    [[nodiscard]] bool recoverFeetAboveIntersectingFloor(
        float worldX, float worldZ, float feetY, float headY,
        float stepHeight, float& outFeetY) const;
