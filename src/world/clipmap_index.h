...
public:
    ...
    [[nodiscard]] const odai::core::CellAabb& worldBounds() const;
...
private:
    ...
    struct ClipmapLevel {
        ...
        std::vector<odai::core::Cell3i> dirtyBrickRingQueue;
    };
    ...
    static BrickCoord worldToBrickCoord(const odai::core::Cell3i& worldCell, std::int32_t brickWorldSize);
    static std::int32_t snapDownToMultiple(std::int32_t value, std::int32_t multiple);
    static odai::core::CellAabb makeLevelBounds(const odai::core::Cell3i& originMin, std::int32_t gridResolution, std::int32_t voxelSize);
    ...
    std::vector<odai::core::CellAabb> m_chunkBounds;
    std::vector<std::size_t> m_allChunkIndices;
    odai::core::CellAabb m_worldBounds{};
