...
struct Brush {
    ...
    odai::core::Cell3i minCell{};
    odai::core::Cell3i maxCell{};
    odai::core::Dir6 axis = odai::core::Dir6::PosY;
...
public:
    CsgVolume() = default;
    CsgVolume(std::int32_t sizeX, std::int32_t sizeY, std::int32_t sizeZ, odai::core::Cell3i originCell = {});

    bool isValid() const;
    odai::core::Cell3i origin() const;
    std::int32_t sizeX() const;
    std::int32_t sizeY() const;
    std::int32_t sizeZ() const;
    odai::core::CellAabb worldBounds() const;

    bool containsWorldCell(const odai::core::Cell3i& worldCell) const;
    CsgCell cellAtWorld(const odai::core::Cell3i& worldCell) const;
    void setCellAtWorld(const odai::core::Cell3i& worldCell, const CsgCell& cell);

    const std::vector<CsgCell>& cells() const;
    std::vector<CsgCell>& cells();

private:
    std::size_t linearIndexFromWorld(const odai::core::Cell3i& worldCell) const;

    odai::core::Cell3i m_origin{};
...
inline CsgVolume::CsgVolume(std::int32_t sizeX, std::int32_t sizeY, std::int32_t sizeZ, odai::core::Cell3i originCell)
    : m_origin(originCell),
...
inline odai::core::Cell3i CsgVolume::origin() const {
    return m_origin;
}
...
inline odai::core::CellAabb CsgVolume::worldBounds() const {
    if (!isValid()) {
        return odai::core::CellAabb{};
    }
    odai::core::CellAabb bounds{};
    bounds.valid = true;
    bounds.minInclusive = m_origin;
    bounds.maxExclusive = m_origin + odai::core::Cell3i{m_sizeX, m_sizeY, m_sizeZ};
    return bounds;
}

inline bool CsgVolume::containsWorldCell(const odai::core::Cell3i& worldCell) const {
    return worldBounds().contains(worldCell);
}

inline CsgCell CsgVolume::cellAtWorld(const odai::core::Cell3i& worldCell) const {
...
inline void CsgVolume::setCellAtWorld(const odai::core::Cell3i& worldCell, const CsgCell& cell) {
    if (!containsWorldCell(worldCell)) {
        return;
    }
...
inline std::size_t CsgVolume::linearIndexFromWorld(const odai::core::Cell3i& worldCell) const {
    const odai::core::Cell3i local = worldCell - m_origin;
    return static_cast<std::size_t>(
        local.x + (m_sizeX * (local.z + (m_sizeZ * local.y)))
    );
}

inline odai::core::CellAabb brushBounds(const Brush& brush) {
    const odai::core::Cell3i minCell{
        std::min(brush.minCell.x, brush.maxCell.x),
        std::min(brush.minCell.y, brush.maxCell.y),
        std::min(brush.minCell.z, brush.maxCell.z)
    };
    const odai::core::Cell3i maxCell{
...
inline bool brushContainsPrismPipeCell(const Brush& brush, const odai::core::CellAabb& bounds, const odai::core::Cell3i& cell) {
    ...
    case odai::core::Dir6::PosY:
    case odai::core::Dir6::NegY:
...
inline bool brushContainsRampCell(const Brush& brush, const odai::core::CellAabb& bounds, const odai::core::Cell3i& cell) {
    if (!bounds.contains(cell)) {
        return false;
    }
    ...
    case odai::core::Dir6::NegX: {
        ...
        const std::int32_t step = brush.axis == odai::core::Dir6::PosX
    ...
    case odai::core::Dir6::NegZ: {
        ...
        const std::int32_t step = brush.axis == odai::core::Dir6::PosZ
...
inline bool brushContainsCell(const Brush& brush, const odai::core::CellAabb& bounds, const odai::core::Cell3i& cell) {
    switch (brush.kind) {
    case BrushKind::Box:
...
inline odai::core::CellAabb applyCsgCommand(CsgVolume& volume, const CsgCommand& command) {
    odai::core::CellAabb touched{};
    if (!volume.isValid()) {
        return touched;
    }

    const odai::core::CellAabb bounds = odai::core::intersectAabb(volume.worldBounds(), brushBounds(command.brush));
    ...
    for (std::int32_t y = bounds.minInclusive.y; y < bounds.maxExclusive.y; ++y) {
        for (std::int32_t z = bounds.minInclusive.z; z < bounds.maxExclusive.z; ++z) {
            for (std::int32_t x = bounds.minInclusive.x; x < bounds.maxExclusive.x; ++x) {
                const odai::core::Cell3i worldCell{x, y, z};
...
inline odai::core::CellAabb applyCsgCommands(CsgVolume& volume, const std::vector<CsgCommand>& commands) {
    odai::core::CellAabb touched{};
...
inline odai::core::CellAabb copyVolumeSolidsToChunk(const CsgVolume& volume, Chunk& chunk) {
    odai::core::CellAabb touched{};
    if (!volume.isValid()) {
        return touched;
    }

    const odai::core::Cell3i chunkOrigin{
        chunk.chunkX() * Chunk::kSizeX,
        chunk.chunkY() * Chunk::kSizeY,
        chunk.chunkZ() * Chunk::kSizeZ
    };
    odai::core::CellAabb chunkBounds{};
    chunkBounds.valid = true;
    chunkBounds.minInclusive = chunkOrigin;
    chunkBounds.maxExclusive = chunkOrigin + odai::core::Cell3i{Chunk::kSizeX, Chunk::kSizeY, Chunk::kSizeZ};

    const odai::core::CellAabb overlap = odai::core::intersectAabb(volume.worldBounds(), chunkBounds);
