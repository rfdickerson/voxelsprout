...
struct Socket {
    ...
    odai::core::Dir6 face = odai::core::Dir6::PosY;
    std::uint8_t lane = 0;
...
struct EdgeSpan {
    odai::core::Cell3i start{};
    odai::core::Dir6 dir = odai::core::Dir6::PosY;
...
inline constexpr odai::core::Cell3i spanEndCell(const EdgeSpan& span) {
    if (!isValidEdgeSpan(span)) {
        return span.start;
    }
    const std::int32_t steps = static_cast<std::int32_t>(span.lengthVoxels) - 1;
    return span.start + (odai::core::dirToOffset(span.dir) * steps);
}
