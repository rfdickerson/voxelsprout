...
std::uint8_t neighborMask6(const odai::core::Cell3i& cell, OccupancyFn&& isOccupied) {
    std::uint8_t mask = 0;
    for (const odai::core::Dir6 dir : odai::core::kAllDir6) {
        if (isOccupied(odai::core::neighborCell(cell, dir))) {
            mask = static_cast<std::uint8_t>(mask | odai::core::dirBit(dir));
        }
    }
    return mask;
}

inline std::vector<odai::core::Cell3i> rasterizeSpan(const EdgeSpan& span) {
    std::vector<odai::core::Cell3i> cells;
    if (!isValidEdgeSpan(span)) {
        return cells;
    }

    cells.reserve(static_cast<std::size_t>(span.lengthVoxels));
    odai::core::Cell3i cursor = span.start;
    const odai::core::Cell3i step = odai::core::dirToOffset(span.dir);
...
inline JoinPiece classifyJoinPiece(std::uint8_t neighborMask) {
    ...
    if (degree == 2u) {
        odai::core::Dir6 first = odai::core::Dir6::PosX;
        odai::core::Dir6 second = odai::core::Dir6::PosX;
        ...
        return odai::core::areOpposite(first, second) ? JoinPiece::Straight : JoinPiece::Elbow;
    }
    if (degree == 3u) {
...
inline QuantizedTransform quantizeTransform(const odai::math::Vector3& positionMeters, const odai::math::Vector3& eulerDegrees) {
...
inline odai::math::Vector3 dequantizePosition(const QuantizedTransform& transform) {
...
inline odai::math::Vector3 dequantizeEulerDegrees(const QuantizedTransform& transform) {
    return odai::math::Vector3{
