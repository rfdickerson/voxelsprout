...
constexpr std::uint32_t kCloudDetailNoiseVolumeDim = 64;
constexpr std::uint32_t kDensityVolumeDim = 128;
constexpr std::uint32_t kSunTransmittanceVolumeDim = 64;
constexpr std::uint32_t kSunTransmittanceMarchSteps = 32;
constexpr std::uint32_t kMultiScatterVolumeDim = 32;
constexpr std::uint32_t kMultiScatterIterationsHigh = 6;
constexpr std::uint32_t kMultiScatterIterationsLow = 2;
constexpr float kMultiScatterLambda = 0.45f;

constexpr const char* kCloudShaderPath = "shaders/cloud_path_trace.comp.slang.spv";
constexpr const char* kDensityBakeShaderPath = "shaders/cloud_density_bake.comp.slang.spv";
constexpr const char* kSunTransmittanceShaderPath = "shaders/cloud_sun_transmittance.comp.slang.spv";
constexpr const char* kMultiScatterShaderPath = "shaders/cloud_multiscatter.comp.slang.spv";
...
std::vector<std::uint8_t> generateCloudDetailNoiseVolumeRgba8(std::uint32_t dim) {
    const std::size_t voxelCount = static_cast<std::size_t>(dim) * static_cast<std::size_t>(dim) * static_cast<std::size_t>(dim);
    std::vector<std::uint8_t> data(voxelCount * 4u, 0u);

    const int freqWorley1 = 12;
    const int freqWorley2 = 24;
    const int freqWorley3 = 48;
    const int freqFbm = 10;
    ...
    for (std::uint32_t z = 0; z < dim; ++z) {
        for (std::uint32_t y = 0; y < dim; ++y) {
            for (std::uint32_t x = 0; x < dim; ++x) {
                ...
                const float fbm = fbmValueNoisePeriodic(nx, ny, nz, freqFbm, 5, 2.11f, 0.5f, 0xD5E1u);
                const float worley1 = 1.0f - worleyPeriodic(nx, ny, nz, freqWorley1, 0xD7A1u);
                const float worley2 = 1.0f - worleyPeriodic(nx, ny, nz, freqWorley2, 0xE8B2u);
                const float worley3 = 1.0f - worleyPeriodic(nx, ny, nz, freqWorley3, 0xF9C3u);

                const float d0 = std::clamp(worley1 * 0.6f + worley2 * 0.3f + worley3 * 0.1f, 0.0f, 1.0f);
                const float d1 = std::clamp(worley2 * 0.6f + worley3 * 0.3f + worley1 * 0.1f, 0.0f, 1.0f);
                const float d2 = std::clamp(worley3 * 0.7f + worley2 * 0.3f, 0.0f, 1.0f);
                const float d3 = std::clamp(fbm * 0.55f + d0 * 0.45f, 0.0f, 1.0f);

                const std::size_t idx = (static_cast<std::size_t>(z) * static_cast<std::size_t>(dim) * static_cast<std::size_t>(dim)
                    + static_cast<std::size_t>(y) * static_cast<std::size_t>(dim)
                    + static_cast<std::size_t>(x)) * 4u;
                data[idx + 0] = static_cast<std::uint8_t>(d0 * 255.0f + 0.5f);
                data[idx + 1] = static_cast<std::uint8_t>(d1 * 255.0f + 0.5f);
                data[idx + 2] = static_cast<std::uint8_t>(d2 * 255.0f + 0.5f);
                data[idx + 3] = static_cast<std::uint8_t>(d3 * 255.0f + 0.5f);
            }
        }
    }
    return data;
...
bool paramsDiffer(const RenderParameters& a, const RenderParameters& b) {
    ...
    return
        ...
        a.densityBakeEveryFrame != b.densityBakeEveryFrame ||
        a.multiScatterTemporalMode != b.multiScatterTemporalMode ||
...
struct Renderer::Impl {
    ...
    ImageResource densityEdgeImage;
    ...
    ImageResource cloudDetailNoiseImage;
    ...
    DensityBakePass densityBakePass;
    SunTransmittancePass sunTransmittancePass;
    MultiScatterPass multiScatterPass;
...
bool Renderer::Impl::createStorageImages() {
    ...
    if (!createImage3D(
            cloudDetailNoiseImage,
            VK_FORMAT_R8G8B8A8_UNORM,
            VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            kCloudDetailNoiseVolumeDim,
            kCloudDetailNoiseVolumeDim,
            kCloudDetailNoiseVolumeDim)) {
        return false;
    }
    ...
    if (!createImage3D(
            sunTransmittanceImage,
            VK_FORMAT_R16_SFLOAT,
            VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            kSunTransmittanceVolumeDim,
            kSunTransmittanceVolumeDim,
            kSunTransmittanceVolumeDim)) {
        return false;
    }
    ...
    const std::vector<std::uint8_t> cloudDetailNoiseData = generateCloudDetailNoiseVolumeRgba8(kCloudDetailNoiseVolumeDim);
    const std::size_t cloudDetailNoiseByteCount = cloudDetailNoiseData.size();

    VkBuffer cloudDetailStagingBuffer = VK_NULL_HANDLE;
    VmaAllocation cloudDetailStagingAllocation = VK_NULL_HANDLE;
    VmaAllocationInfo cloudDetailStagingAllocInfo{};

    VkBufferCreateInfo cloudDetailStagingInfo{};
    cloudDetailStagingInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    cloudDetailStagingInfo.size = cloudDetailNoiseByteCount;
    cloudDetailStagingInfo.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
    cloudDetailStagingInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

    VmaAllocationCreateInfo cloudDetailStagingCreateInfo{};
    cloudDetailStagingCreateInfo.usage = VMA_MEMORY_USAGE_AUTO;
    cloudDetailStagingCreateInfo.flags = VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT | VMA_ALLOCATION_CREATE_MAPPED_BIT;
    if (vmaCreateBuffer(
            allocator,
            &cloudDetailStagingInfo,
            &cloudDetailStagingCreateInfo,
            &cloudDetailStagingBuffer,
            &cloudDetailStagingAllocation,
            &cloudDetailStagingAllocInfo) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to create cloud detail noise staging buffer";
        return false;
    }

    void* cloudDetailMappedData = cloudDetailStagingAllocInfo.pMappedData;
    bool cloudDetailDidMap = false;
    if (cloudDetailMappedData == nullptr) {
        if (vmaMapMemory(allocator, cloudDetailStagingAllocation, &cloudDetailMappedData) != VK_SUCCESS) {
            VOX_LOGE("render") << "failed to map cloud detail noise staging buffer";
            vmaDestroyBuffer(allocator, cloudDetailStagingBuffer, cloudDetailStagingAllocation);
            return false;
        }
        cloudDetailDidMap = true;
    }
    std::memcpy(cloudDetailMappedData, cloudDetailNoiseData.data(), cloudDetailNoiseByteCount);
    if (cloudDetailDidMap) {
        vmaUnmapMemory(allocator, cloudDetailStagingAllocation);
    }

    VkCommandPool cloudDetailUploadPool = VK_NULL_HANDLE;
    VkCommandBuffer cloudDetailUploadCmd = VK_NULL_HANDLE;

    VkCommandPoolCreateInfo cloudDetailUploadPoolInfo{};
    cloudDetailUploadPoolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
    cloudDetailUploadPoolInfo.queueFamilyIndex = computeQueueFamilyIndex;
    cloudDetailUploadPoolInfo.flags = VK_COMMAND_POOL_CREATE_TRANSIENT_BIT;
    if (vkCreateCommandPool(device, &cloudDetailUploadPoolInfo, nullptr, &cloudDetailUploadPool) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to create cloud detail upload command pool";
        vmaDestroyBuffer(allocator, cloudDetailStagingBuffer, cloudDetailStagingAllocation);
        return false;
    }

    VkCommandBufferAllocateInfo cloudDetailUploadAllocInfo{};
    cloudDetailUploadAllocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    cloudDetailUploadAllocInfo.commandPool = cloudDetailUploadPool;
    cloudDetailUploadAllocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    cloudDetailUploadAllocInfo.commandBufferCount = 1;
    if (vkAllocateCommandBuffers(device, &cloudDetailUploadAllocInfo, &cloudDetailUploadCmd) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to allocate cloud detail upload command buffer";
        vkDestroyCommandPool(device, cloudDetailUploadPool, nullptr);
        vmaDestroyBuffer(allocator, cloudDetailStagingBuffer, cloudDetailStagingAllocation);
        return false;
    }

    VkCommandBufferBeginInfo cloudDetailBeginInfo{};
    cloudDetailBeginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    cloudDetailBeginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
    if (vkBeginCommandBuffer(cloudDetailUploadCmd, &cloudDetailBeginInfo) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to begin cloud detail upload command buffer";
        vkDestroyCommandPool(device, cloudDetailUploadPool, nullptr);
        vmaDestroyBuffer(allocator, cloudDetailStagingBuffer, cloudDetailStagingAllocation);
        return false;
    }

    VkImageMemoryBarrier2 cloudDetailToTransfer = imageBarrier(
        cloudDetailNoiseImage.image,
        VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        VK_PIPELINE_STAGE_2_NONE,
        VK_PIPELINE_STAGE_2_TRANSFER_BIT,
        VK_ACCESS_2_NONE,
        VK_ACCESS_2_TRANSFER_WRITE_BIT);
    VkDependencyInfo cloudDetailToTransferDep{};
    cloudDetailToTransferDep.sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO;
    cloudDetailToTransferDep.imageMemoryBarrierCount = 1;
    cloudDetailToTransferDep.pImageMemoryBarriers = &cloudDetailToTransfer;
    vkCmdPipelineBarrier2(cloudDetailUploadCmd, &cloudDetailToTransferDep);

    VkBufferImageCopy cloudDetailCopyRegion{};
    cloudDetailCopyRegion.bufferOffset = 0;
    cloudDetailCopyRegion.bufferRowLength = 0;
    cloudDetailCopyRegion.bufferImageHeight = 0;
    cloudDetailCopyRegion.imageSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    cloudDetailCopyRegion.imageSubresource.mipLevel = 0;
    cloudDetailCopyRegion.imageSubresource.baseArrayLayer = 0;
    cloudDetailCopyRegion.imageSubresource.layerCount = 1;
    cloudDetailCopyRegion.imageExtent = {kCloudDetailNoiseVolumeDim, kCloudDetailNoiseVolumeDim, kCloudDetailNoiseVolumeDim};
    vkCmdCopyBufferToImage(
        cloudDetailUploadCmd,
        cloudDetailStagingBuffer,
        cloudDetailNoiseImage.image,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        1,
        &cloudDetailCopyRegion);

    VkImageMemoryBarrier2 cloudDetailToSample = imageBarrier(
        cloudDetailNoiseImage.image,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
        VK_PIPELINE_STAGE_2_TRANSFER_BIT,
        ...
        VK_ACCESS_2_TRANSFER_WRITE_BIT,
        VK_ACCESS_2_SHADER_SAMPLED_READ_BIT);
    VkDependencyInfo cloudDetailToSampleDep{};
    cloudDetailToSampleDep.sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO;
    cloudDetailToSampleDep.imageMemoryBarrierCount = 1;
    cloudDetailToSampleDep.pImageMemoryBarriers = &cloudDetailToSample;
    vkCmdPipelineBarrier2(cloudDetailUploadCmd, &cloudDetailToSampleDep);

    if (vkEndCommandBuffer(cloudDetailUploadCmd) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to end cloud detail upload command buffer";
        vkDestroyCommandPool(device, cloudDetailUploadPool, nullptr);
        vmaDestroyBuffer(allocator, cloudDetailStagingBuffer, cloudDetailStagingAllocation);
        return false;
    }

    VkSubmitInfo cloudDetailSubmitInfo{};
    cloudDetailSubmitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
    cloudDetailSubmitInfo.commandBufferCount = 1;
    cloudDetailSubmitInfo.pCommandBuffers = &cloudDetailUploadCmd;
    if (vkQueueSubmit(renderQueue, 1, &cloudDetailSubmitInfo, VK_NULL_HANDLE) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to submit cloud detail upload";
        vkDestroyCommandPool(device, cloudDetailUploadPool, nullptr);
        vmaDestroyBuffer(allocator, cloudDetailStagingBuffer, cloudDetailStagingAllocation);
        return false;
    }
    ...
    vkDestroyCommandPool(device, cloudDetailUploadPool, nullptr);
    vmaDestroyBuffer(allocator, cloudDetailStagingBuffer, cloudDetailStagingAllocation);
    cloudDetailNoiseImage.layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
...
bool Renderer::Impl::createDescriptors() {
    ...
    std::array<VkDescriptorSetLayoutBinding, 10> cloudBindings{};
    ...
    cloudBindings[5].binding = 5;
    cloudBindings[5].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    cloudBindings[5].descriptorCount = 1;
    cloudBindings[5].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    cloudBindings[6].binding = 6;
    cloudBindings[6].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    cloudBindings[6].descriptorCount = 1;
    cloudBindings[6].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    cloudBindings[7].binding = 7;
    ...
    cloudBindings[9].binding = 9;
    cloudBindings[9].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    cloudBindings[9].descriptorCount = 1;
    cloudBindings[9].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    ...
    std::array<VkDescriptorSetLayoutBinding, 5> densityBindings{};
    densityBindings[0].binding = 0;
    densityBindings[0].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
    densityBindings[0].descriptorCount = 1;
    densityBindings[0].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    densityBindings[1].binding = 1;
    densityBindings[1].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    densityBindings[1].descriptorCount = 1;
    densityBindings[1].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    densityBindings[2].binding = 2;
    densityBindings[2].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    densityBindings[2].descriptorCount = 1;
    densityBindings[2].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    densityBindings[3].binding = 3;
    densityBindings[3].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLER;
    densityBindings[3].descriptorCount = 1;
    densityBindings[3].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    densityBindings[4].binding = 4;
    densityBindings[4].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
    densityBindings[4].descriptorCount = 1;
    densityBindings[4].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;

    VkDescriptorSetLayoutCreateInfo densityLayoutInfo{};
    densityLayoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    densityLayoutInfo.bindingCount = static_cast<std::uint32_t>(densityBindings.size());
    densityLayoutInfo.pBindings = densityBindings.data();

    if (vkCreateDescriptorSetLayout(device, &densityLayoutInfo, nullptr, &densityBakePass.descriptorSetLayout) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to create density-bake descriptor set layout";
        return false;
    }

    std::array<VkDescriptorSetLayoutBinding, 3> sunBindings{};
    sunBindings[0].binding = 0;
    sunBindings[0].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
    sunBindings[0].descriptorCount = 1;
    sunBindings[0].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    sunBindings[1].binding = 1;
    sunBindings[1].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    sunBindings[1].descriptorCount = 1;
    sunBindings[1].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    sunBindings[2].binding = 2;
    sunBindings[2].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLER;
    sunBindings[2].descriptorCount = 1;
    sunBindings[2].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;

    VkDescriptorSetLayoutCreateInfo sunLayoutInfo{};
    sunLayoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    sunLayoutInfo.bindingCount = static_cast<std::uint32_t>(sunBindings.size());
    sunLayoutInfo.pBindings = sunBindings.data();

    if (vkCreateDescriptorSetLayout(device, &sunLayoutInfo, nullptr, &sunTransmittancePass.descriptorSetLayout) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to create sun transmittance descriptor set layout";
        return false;
    }

    std::array<VkDescriptorSetLayoutBinding, 5> multiScatterBindings{};
    multiScatterBindings[0].binding = 0;
    multiScatterBindings[0].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
    multiScatterBindings[0].descriptorCount = 1;
    multiScatterBindings[0].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    multiScatterBindings[1].binding = 1;
    multiScatterBindings[1].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    multiScatterBindings[1].descriptorCount = 1;
    multiScatterBindings[1].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    multiScatterBindings[2].binding = 2;
    multiScatterBindings[2].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    multiScatterBindings[2].descriptorCount = 1;
    multiScatterBindings[2].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    multiScatterBindings[3].binding = 3;
    multiScatterBindings[3].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    multiScatterBindings[3].descriptorCount = 1;
    multiScatterBindings[3].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    multiScatterBindings[4].binding = 4;
    multiScatterBindings[4].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLER;
    multiScatterBindings[4].descriptorCount = 1;
    multiScatterBindings[4].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;

    VkDescriptorSetLayoutCreateInfo multiScatterLayoutInfo{};
    multiScatterLayoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    multiScatterLayoutInfo.bindingCount = static_cast<std::uint32_t>(multiScatterBindings.size());
    multiScatterLayoutInfo.pBindings = multiScatterBindings.data();

    if (vkCreateDescriptorSetLayout(device, &multiScatterLayoutInfo, nullptr, &multiScatterPass.descriptorSetLayout) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to create multi-scatter descriptor set layout";
        return false;
    }
    ...
    std::array<VkDescriptorSetLayout, 1> densitySetLayouts = {densityBakePass.descriptorSetLayout};
    VkDescriptorSetAllocateInfo densityAllocInfo{};
    densityAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    densityAllocInfo.descriptorPool = descriptorPool;
    densityAllocInfo.descriptorSetCount = static_cast<std::uint32_t>(densitySetLayouts.size());
    densityAllocInfo.pSetLayouts = densitySetLayouts.data();

    if (vkAllocateDescriptorSets(device, &densityAllocInfo, &densityBakePass.descriptorSet) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to allocate density-bake descriptor set";
        return false;
    }

    std::array<VkDescriptorSetLayout, 1> sunSetLayouts = {sunTransmittancePass.descriptorSetLayout};
    VkDescriptorSetAllocateInfo sunAllocInfo{};
    sunAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    sunAllocInfo.descriptorPool = descriptorPool;
    sunAllocInfo.descriptorSetCount = static_cast<std::uint32_t>(sunSetLayouts.size());
    sunAllocInfo.pSetLayouts = sunSetLayouts.data();

    if (vkAllocateDescriptorSets(device, &sunAllocInfo, &sunTransmittancePass.descriptorSet) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to allocate sun transmittance descriptor set";
        return false;
    }

    std::array<VkDescriptorSetLayout, 2> multiScatterSetLayouts = {
        multiScatterPass.descriptorSetLayout,
        multiScatterPass.descriptorSetLayout};
    VkDescriptorSetAllocateInfo multiScatterAllocInfo{};
    multiScatterAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    multiScatterAllocInfo.descriptorPool = descriptorPool;
    multiScatterAllocInfo.descriptorSetCount = static_cast<std::uint32_t>(multiScatterSetLayouts.size());
    multiScatterAllocInfo.pSetLayouts = multiScatterSetLayouts.data();

    if (vkAllocateDescriptorSets(device, &multiScatterAllocInfo, multiScatterPass.descriptorSets.data()) != VK_SUCCESS) {
        VOX_LOGE("render") << "failed to allocate multi-scatter descriptor sets";
        return false;
    }
    ...
    VkDescriptorImageInfo cloudDetailNoiseInfo{};
    cloudDetailNoiseInfo.sampler = VK_NULL_HANDLE;
    cloudDetailNoiseInfo.imageView = cloudDetailNoiseImage.view;
    cloudDetailNoiseInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    ...
    VkDescriptorImageInfo sunTransmittanceStorageInfo{};
    sunTransmittanceStorageInfo.imageView = sunTransmittanceImage.view;
    sunTransmittanceStorageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

    VkDescriptorImageInfo sunTransmittanceSampleInfo{};
    sunTransmittanceSampleInfo.sampler = VK_NULL_HANDLE;
    sunTransmittanceSampleInfo.imageView = sunTransmittanceImage.view;
    sunTransmittanceSampleInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

    VkDescriptorImageInfo densityStorageInfo{};
    densityStorageInfo.imageView = densityImage.view;
    densityStorageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

    VkDescriptorImageInfo densityEdgeStorageInfo{};
    densityEdgeStorageInfo.imageView = densityEdgeImage.view;
    densityEdgeStorageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

    VkDescriptorImageInfo densitySampleInfo{};
    densitySampleInfo.sampler = VK_NULL_HANDLE;
    densitySampleInfo.imageView = densityImage.view;
    densitySampleInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

    VkDescriptorImageInfo densityEdgeSampleInfo{};
    densityEdgeSampleInfo.sampler = VK_NULL_HANDLE;
    densityEdgeSampleInfo.imageView = densityEdgeImage.view;
    densityEdgeSampleInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
    ...
    multiScatterSampleInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

    VkDescriptorImageInfo multiScatterAStorageInfo{};
    multiScatterAStorageInfo.imageView = multiScatterImages[0].view;
    multiScatterAStorageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

    VkDescriptorImageInfo multiScatterBStorageInfo{};
    multiScatterBStorageInfo.imageView = multiScatterImages[1].view;
    multiScatterBStorageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
    ...
    writes[5].dstSet = cloudPathTracePass.descriptorSet;
    writes[5].dstBinding = 5;
    writes[5].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    writes[5].descriptorCount = 1;
    writes[5].pImageInfo = &sunTransmittanceSampleInfo;

    writes[6].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writes[6].dstSet = cloudPathTracePass.descriptorSet;
    writes[6].dstBinding = 6;
    writes[6].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
    writes[6].descriptorCount = 1;
    writes[6].pImageInfo = &multiScatterSampleInfo;

    writes[7].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writes[7].dstSet = cloudPathTracePass.descriptorSet;
    ...
    writes[11].pImageInfo = &cloudDetailNoiseInfo;

    writes[12].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writes[12].dstSet = densityBakePass.descriptorSet;
    writes[12].dstBinding = 3;
    writes[12].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLER;
    writes[12].descriptorCount = 1;
    writes[12].pImageInfo = &cloudNoiseSamplerInfo;
    ...
    writes[13].pImageInfo = &sunTransmittanceStorageInfo;
    ...
    writes[14].pImageInfo = &densitySampleInfo;

    writes[15].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writes[15].dstSet = sunTransmittancePass.descriptorSet;
    writes[15].dstBinding = 2;
    writes[15].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLER;
    writes[15].descriptorCount = 1;
    writes[15].pImageInfo = &cloudNoiseSamplerInfo;
    ...
    writes[16].pImageInfo = &multiScatterBStorageInfo;
    ...
    writes[21].pImageInfo = &multiScatterAStorageInfo;
...
void Renderer::Impl::destroyDescriptors() {
    ...
    if (densityBakePass.descriptorSetLayout != VK_NULL_HANDLE) {
        vkDestroyDescriptorSetLayout(device, densityBakePass.descriptorSetLayout, nullptr);
        densityBakePass.descriptorSetLayout = VK_NULL_HANDLE;
    }
    if (sunTransmittancePass.descriptorSetLayout != VK_NULL_HANDLE) {
        vkDestroyDescriptorSetLayout(device, sunTransmittancePass.descriptorSetLayout, nullptr);
        sunTransmittancePass.descriptorSetLayout = VK_NULL_HANDLE;
    }
    if (multiScatterPass.descriptorSetLayout != VK_NULL_HANDLE) {
        vkDestroyDescriptorSetLayout(device, multiScatterPass.descriptorSetLayout, nullptr);
        multiScatterPass.descriptorSetLayout = VK_NULL_HANDLE;
    }
...
bool Renderer::Impl::createPipelines() {
    ...
    VkShaderModule densityModule = createShaderModuleFromSpv(kDensityBakeShaderPath);
    ...
    VkPushConstantRange densityPushRange{};
    densityPushRange.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    densityPushRange.offset = 0;
    densityPushRange.size = sizeof(DensityBakePush);

    VkPipelineLayoutCreateInfo densityLayoutInfo{};
    densityLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    densityLayoutInfo.setLayoutCount = 1;
    densityLayoutInfo.pSetLayouts = &densityBakePass.descriptorSetLayout;
    densityLayoutInfo.pushConstantRangeCount = 1;
    densityLayoutInfo.pPushConstantRanges = &densityPushRange;
    ...
    VkPushConstantRange sunPushRange{};
    sunPushRange.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    sunPushRange.offset = 0;
    sunPushRange.size = sizeof(SunTransmittancePush);

    VkPipelineLayoutCreateInfo sunLayoutInfo{};
    sunLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    sunLayoutInfo.setLayoutCount = 1;
    sunLayoutInfo.pSetLayouts = &sunTransmittancePass.descriptorSetLayout;
    sunLayoutInfo.pushConstantRangeCount = 1;
    sunLayoutInfo.pPushConstantRanges = &sunPushRange;
    ...
    VkPushConstantRange multiScatterPushRange{};
    multiScatterPushRange.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    multiScatterPushRange.offset = 0;
    multiScatterPushRange.size = sizeof(MultiScatterPush);

    VkPipelineLayoutCreateInfo multiScatterLayoutInfo{};
    multiScatterLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    multiScatterLayoutInfo.setLayoutCount = 1;
    multiScatterLayoutInfo.pSetLayouts = &multiScatterPass.descriptorSetLayout;
    multiScatterLayoutInfo.pushConstantRangeCount = 1;
    multiScatterLayoutInfo.pPushConstantRanges = &multiScatterPushRange;
    ...
    densityPipelineInfo.stage = densityStage;
    densityPipelineInfo.layout = densityBakePass.pipelineLayout;
    ...
    VkPipelineShaderStageCreateInfo sunStage{};
    sunStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    sunStage.stage = VK_SHADER_STAGE_COMPUTE_BIT;
    sunStage.module = sunTransmittanceModule;
    sunStage.pName = "main";

    VkComputePipelineCreateInfo sunPipelineInfo{};
    sunPipelineInfo.sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO;
    sunPipelineInfo.stage = sunStage;
    sunPipelineInfo.layout = sunTransmittancePass.pipelineLayout;
    ...
    VkPipelineShaderStageCreateInfo multiScatterStage{};
    multiScatterStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    multiScatterStage.stage = VK_SHADER_STAGE_COMPUTE_BIT;
    multiScatterStage.module = multiScatterModule;
    multiScatterStage.pName = "main";

    VkComputePipelineCreateInfo multiScatterPipelineInfo{};
    multiScatterPipelineInfo.sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO;
    multiScatterPipelineInfo.stage = multiScatterStage;
    multiScatterPipelineInfo.layout = multiScatterPass.pipelineLayout;
...
void Renderer::Impl::destroyPipelines() {
    ...
    if (densityBakePass.pipeline != VK_NULL_HANDLE) {
        vkDestroyPipeline(device, densityBakePass.pipeline, nullptr);
        densityBakePass.pipeline = VK_NULL_HANDLE;
    }
    if (densityBakePass.pipelineLayout != VK_NULL_HANDLE) {
        vkDestroyPipelineLayout(device, densityBakePass.pipelineLayout, nullptr);
        densityBakePass.pipelineLayout = VK_NULL_HANDLE;
    }
    if (sunTransmittancePass.pipeline != VK_NULL_HANDLE) {
        vkDestroyPipeline(device, sunTransmittancePass.pipeline, nullptr);
        sunTransmittancePass.pipeline = VK_NULL_HANDLE;
    }
    if (sunTransmittancePass.pipelineLayout != VK_NULL_HANDLE) {
        vkDestroyPipelineLayout(device, sunTransmittancePass.pipelineLayout, nullptr);
        sunTransmittancePass.pipelineLayout = VK_NULL_HANDLE;
    }
    if (multiScatterPass.pipeline != VK_NULL_HANDLE) {
        vkDestroyPipeline(device, multiScatterPass.pipeline, nullptr);
        multiScatterPass.pipeline = VK_NULL_HANDLE;
    }
    if (multiScatterPass.pipelineLayout != VK_NULL_HANDLE) {
        vkDestroyPipelineLayout(device, multiScatterPass.pipelineLayout, nullptr);
        multiScatterPass.pipelineLayout = VK_NULL_HANDLE;
    }
...
bool Renderer::Impl::render(const RenderParameters& params) {
    ...
    if (shouldSubmitCompute) {
        ...
        std::array<VkImageMemoryBarrier2, 9> startupBarriers{};
        ...
        const VkAccessFlags2 sunSrcAccess =
            (sunTransmittanceImage.layout == VK_IMAGE_LAYOUT_UNDEFINED) ? VK_ACCESS_2_NONE : VK_ACCESS_2_MEMORY_WRITE_BIT;
        startupBarriers[startupBarrierCount++] = imageBarrier(
            sunTransmittanceImage.image,
            sunTransmittanceImage.layout,
            VK_IMAGE_LAYOUT_GENERAL,
            sunSrcStage,
            VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
            sunSrcAccess,
        ...
        const VkPipelineStageFlags2 densitySrcStage =
            (densityImage.layout == VK_IMAGE_LAYOUT_UNDEFINED) ? VK_PIPELINE_STAGE_2_NONE : VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT;
        const VkAccessFlags2 densitySrcAccess =
            (densityImage.layout == VK_IMAGE_LAYOUT_UNDEFINED) ? VK_ACCESS_2_NONE : VK_ACCESS_2_MEMORY_WRITE_BIT;
        startupBarriers[startupBarrierCount++] = imageBarrier(
            densityImage.image,
            densityImage.layout,
            VK_IMAGE_LAYOUT_GENERAL,
        ...
        const VkPipelineStageFlags2 densityEdgeSrcStage =
            (densityEdgeImage.layout == VK_IMAGE_LAYOUT_UNDEFINED) ? VK_PIPELINE_STAGE_2_NONE : VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT;
        const VkAccessFlags2 densityEdgeSrcAccess =
            (densityEdgeImage.layout == VK_IMAGE_LAYOUT_UNDEFINED) ? VK_ACCESS_2_NONE : VK_ACCESS_2_MEMORY_WRITE_BIT;
        startupBarriers[startupBarrierCount++] = imageBarrier(
            densityEdgeImage.image,
            densityEdgeImage.layout,
            VK_IMAGE_LAYOUT_GENERAL,
            densityEdgeSrcStage,
            VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
            densityEdgeSrcAccess,
            VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT | VK_ACCESS_2_SHADER_SAMPLED_READ_BIT);

        for (ImageResource& multiScatterImage : multiScatterImages) {
            const VkPipelineStageFlags2 multiSrcStage =
                (multiScatterImage.layout == VK_IMAGE_LAYOUT_UNDEFINED) ? VK_PIPELINE_STAGE_2_NONE : VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT;
            const VkAccessFlags2 multiSrcAccess =
                (multiScatterImage.layout == VK_IMAGE_LAYOUT_UNDEFINED) ? VK_ACCESS_2_NONE : VK_ACCESS_2_MEMORY_WRITE_BIT;
            startupBarriers[startupBarrierCount++] = imageBarrier(
                multiScatterImage.image,
                multiScatterImage.layout,
                VK_IMAGE_LAYOUT_GENERAL,
                multiSrcStage,
                VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
                multiSrcAccess,
        ...
        densityEdgeImage.layout = VK_IMAGE_LAYOUT_GENERAL;
        multiScatterImages[0].layout = VK_IMAGE_LAYOUT_GENERAL;
        multiScatterImages[1].layout = VK_IMAGE_LAYOUT_GENERAL;
        toneMapOwnedByUi = false;
        ...
        if (resetAccumulation) {
            ...
            std::array<VkImageMemoryBarrier2, 2> clearToCompute{};
            clearToCompute[0] = imageBarrier(
                multiScatterImages[0].image,
                VK_IMAGE_LAYOUT_GENERAL,
                VK_IMAGE_LAYOUT_GENERAL,
                VK_PIPELINE_STAGE_2_TRANSFER_BIT,
                VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
                VK_ACCESS_2_TRANSFER_WRITE_BIT,
                VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT | VK_ACCESS_2_SHADER_SAMPLED_READ_BIT);
            clearToCompute[1] = imageBarrier(
                multiScatterImages[1].image,
                VK_IMAGE_LAYOUT_GENERAL,
                VK_IMAGE_LAYOUT_GENERAL,
                VK_PIPELINE_STAGE_2_TRANSFER_BIT,
                VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
            ...
            clearDep.imageMemoryBarrierCount = static_cast<std::uint32_t>(clearToCompute.size());
            clearDep.pImageMemoryBarriers = clearToCompute.data();
            vkCmdPipelineBarrier2(frame.commandBuffer, &clearDep);
        }
        ...
        if (runDensityBakeThisFrame) {
            vkCmdBindPipeline(frame.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, densityBakePass.pipeline);
            vkCmdBindDescriptorSets(
                frame.commandBuffer,
                VK_PIPELINE_BIND_POINT_COMPUTE,
                densityBakePass.pipelineLayout,
                0,
                1,
                &densityBakePass.descriptorSet,
                0,
                nullptr);

            DensityBakePush densityPush{};
            ...
            densityPush.cloudShapeParams[3] = 0.0f;
            densityPush.cloudProfileParams[0] = params.scene.volume.coverage;
            densityPush.cloudProfileParams[1] = params.scene.volume.weatherScale;
            densityPush.cloudProfileParams[2] = params.scene.volume.cloudBase;
            ...
            densityPush.volumeParams[2] = 0.0f;
            densityPush.volumeParams[3] = 0.0f;

            const std::uint32_t densityDispatch = (kDensityVolumeDim + 3u) / 4u;
            vkCmdPushConstants(
                frame.commandBuffer,
                densityBakePass.pipelineLayout,
                VK_SHADER_STAGE_COMPUTE_BIT,
                0,
                sizeof(DensityBakePush),
                &densityPush);
            vkCmdDispatch(frame.commandBuffer, densityDispatch, densityDispatch, densityDispatch);

            std::array<VkImageMemoryBarrier2, 2> densityToConsumersBarriers{};
            densityToConsumersBarriers[0] = imageBarrier(
                densityImage.image,
                VK_IMAGE_LAYOUT_GENERAL,
                VK_IMAGE_LAYOUT_GENERAL,
                VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
                ...
                VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT,
                VK_ACCESS_2_SHADER_SAMPLED_READ_BIT);
            densityToConsumersBarriers[1] = imageBarrier(
                densityEdgeImage.image,
                VK_IMAGE_LAYOUT_GENERAL,
                VK_IMAGE_LAYOUT_GENERAL,
                VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
                ...
                VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT,
                VK_ACCESS_2_SHADER_SAMPLED_READ_BIT);
            VkDependencyInfo densityToConsumersDep{};
            densityToConsumersDep.sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO;
            densityToConsumersDep.imageMemoryBarrierCount = static_cast<std::uint32_t>(densityToConsumersBarriers.size());
            densityToConsumersDep.pImageMemoryBarriers = densityToConsumersBarriers.data();
            vkCmdPipelineBarrier2(frame.commandBuffer, &densityToConsumersDep);
            densityVolumeValid = true;
        }

        vkCmdBindPipeline(frame.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, sunTransmittancePass.pipeline);
        vkCmdBindDescriptorSets(
            frame.commandBuffer,
            VK_PIPELINE_BIND_POINT_COMPUTE,
            sunTransmittancePass.pipelineLayout,
            0,
            1,
            &sunTransmittancePass.descriptorSet,
            0,
            nullptr);

        SunTransmittancePush sunPush{};
        sunPush.sunDirectionIntensity[0] = sunDir.x;
        sunPush.sunDirectionIntensity[1] = sunDir.y;
        sunPush.sunDirectionIntensity[2] = sunDir.z;
        sunPush.sunDirectionIntensity[3] = params.scene.sun.intensity;
        ...
        sunPush.cloudShapeParams[3] = 0.0f;
        sunPush.cloudProfileParams[0] = params.scene.volume.coverage;
        sunPush.cloudProfileParams[1] = params.scene.volume.weatherScale;
        sunPush.cloudProfileParams[2] = params.scene.volume.cloudBase;
        ...
        sunPush.volumeParams[2] = 0.0f;
        sunPush.volumeParams[3] = 0.0f;

        const std::uint32_t sunDispatch = (kSunTransmittanceVolumeDim + 3u) / 4u;
        vkCmdPushConstants(
            frame.commandBuffer,
            sunTransmittancePass.pipelineLayout,
            VK_SHADER_STAGE_COMPUTE_BIT,
            0,
            sizeof(SunTransmittancePush),
            &sunPush);
        vkCmdDispatch(frame.commandBuffer, sunDispatch, sunDispatch, sunDispatch);

        VkImageMemoryBarrier2 sunToCloudBarrier = imageBarrier(
            sunTransmittanceImage.image,
            VK_IMAGE_LAYOUT_GENERAL,
            VK_IMAGE_LAYOUT_GENERAL,
            VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
            ...
            VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT,
            VK_ACCESS_2_SHADER_SAMPLED_READ_BIT);
        VkDependencyInfo sunToCloudDep{};
        sunToCloudDep.sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO;
        sunToCloudDep.imageMemoryBarrierCount = 1;
        sunToCloudDep.pImageMemoryBarriers = &sunToCloudBarrier;
        vkCmdPipelineBarrier2(frame.commandBuffer, &sunToCloudDep);

        vkCmdBindPipeline(frame.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, multiScatterPass.pipeline);
        const std::uint32_t multiDispatch = (kMultiScatterVolumeDim + 3u) / 4u;
        MultiScatterPush multiScatterPush{};
        multiScatterPush.sunDirectionIntensity[0] = sunDir.x;
        multiScatterPush.sunDirectionIntensity[1] = sunDir.y;
        multiScatterPush.sunDirectionIntensity[2] = sunDir.z;
        multiScatterPush.sunDirectionIntensity[3] = params.scene.sun.intensity;
        ...
        multiScatterPush.cloudShapeParams[3] = 0.0f;
        multiScatterPush.cloudProfileParams[0] = params.scene.volume.coverage;
        multiScatterPush.cloudProfileParams[1] = params.scene.volume.weatherScale;
        multiScatterPush.cloudProfileParams[2] = params.scene.volume.cloudBase;
        ...
        multiScatterPush.solveParams[2] = 20.0f / static_cast<float>(kMultiScatterVolumeDim);
        multiScatterPush.solveParams[3] = 0.0f;

        const std::uint32_t multiScatterIterations =
            params.multiScatterTemporalMode ? kMultiScatterIterationsLow : kMultiScatterIterationsHigh;
        for (std::uint32_t iter = 0; iter < multiScatterIterations; ++iter) {
            const std::uint32_t setIndex = iter & 1u;
            vkCmdBindDescriptorSets(
                frame.commandBuffer,
                VK_PIPELINE_BIND_POINT_COMPUTE,
                multiScatterPass.pipelineLayout,
                0,
                1,
                &multiScatterPass.descriptorSets[setIndex],
                0,
                nullptr);
            vkCmdPushConstants(
                frame.commandBuffer,
                ...
                &multiScatterPush);
            vkCmdDispatch(frame.commandBuffer, multiDispatch, multiDispatch, multiDispatch);

            const std::uint32_t writtenIndex = (setIndex == 0u) ? 1u : 0u;
            VkImageMemoryBarrier2 iterBarrier = imageBarrier(
                multiScatterImages[writtenIndex].image,
                VK_IMAGE_LAYOUT_GENERAL,
                VK_IMAGE_LAYOUT_GENERAL,
                VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
                VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
                VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT,
                VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT | VK_ACCESS_2_SHADER_SAMPLED_READ_BIT);
            VkDependencyInfo iterDep{};
            iterDep.sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO;
            iterDep.imageMemoryBarrierCount = 1;
            iterDep.pImageMemoryBarriers = &iterBarrier;
            vkCmdPipelineBarrier2(frame.commandBuffer, &iterDep);
        }
